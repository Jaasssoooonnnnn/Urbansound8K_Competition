...
CONFIG = {
    ...
    "output_path": "./resnet34_fusion_cbam_asp_salience_64", # 新的输出路径
    ...
    "num_workers": 4,
    "lr": 1e-3,
    "num_epochs": 120, # 稍微增加一点 epoch 让大模型收敛
    "sam_rho": 0.05,
    "label_smoothing": 0.1, # 引入 label smoothing
...
# --- 2.3 Attentive Statistics Pooling (ASP) ---
class AttentiveStatsPooling(nn.Module):
    ...
    def forward(self, x):
        # Input x: [Batch, Channels, Freq, Time] -> 需要 collapse Freq
        # 这里 ResNet 的输出通常是 [B, 512, F', T']
        # 我们先对 Freq 维度做平均，保留 Time 维度用于 ASP
        ...
        out = torch.cat([mu, std], dim=1) # [B, 2*C] = [B, 1024]
        return out

# ==========================================
# 3. 融合模型 (ResNet34 + CBAM + ASP + Salience)
# ==========================================
class AudioResNetFusion(nn.Module):
    def __init__(self, num_classes=10):
        ...
        # --- Pooling (ASP) ---
        # ResNet34 final channels = 512. ASP output = 512 * 2 = 1024
        ...
        self.fc_final = nn.Linear(1024 + self.salience_dim, num_classes)
    ...
    def forward(self, x, salience=None):
        ...
        # 2. Backbone with CBAM
        # 手动执行 ResNet 的各个阶段以插入 CBAM
        ...
        # 3. ASP Pooling
        # x shape at this point: [Batch, 512, F, T]
        audio_feats = self.asp(x) # -> [Batch, 1024]
        audio_feats = self.asp_bn(audio_feats)
        audio_feats = self.dropout(audio_feats)
        
        # 4. Salience Injection
        if salience is not None:
            # Check dtype to decide if it's indices (Test/Val) or vectors (Mixup Train)
...
if __name__ == "__main__":
    ...
    print(f"{'='*50}")
    print(f"FUSION MODEL: ResNet34 + CBAM + ASP + Salience + SAM")
    print(f"Combining Attention Mechanisms with Salience Injection")
    print(f"{'='*50}\n")
    ...
    if os.path.exists(test_csv_path):
        ...
        if models_used > 0:
            ...
            save_name = os.path.join(CONFIG['output_path'], "submission_fusion_final.csv")
