...
class UrbanSoundGPUDataset(Dataset):
    ...
    def _preload(self):
        ...
        full_data = torch.cat(all_wavs, dim=0).to(CONFIG['device']) # Float32
...
if __name__ == "__main__":
    ...
    # 不使用 autocast，全程 FP32，确保数值稳定
    optimizer = torch.optim.AdamW(model.parameters(), lr=CONFIG['lr'], weight_decay=1e-2)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=CONFIG['num_epochs'])
    criterion = nn.CrossEntropyLoss()

    best_acc = 0.0
    print("\nStart Training (Stable FP32 Mode)...")
    ...
    for epoch in range(1, CONFIG['num_epochs'] + 1):
        ...
        for wavs, labels in train_loader:
            ...
            # Step 3: 反向传播 (加梯度裁剪)
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0) # 裁剪梯度防止爆炸
            optimizer.step()
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), f"best_baseline_fold{FOLD}.pth")