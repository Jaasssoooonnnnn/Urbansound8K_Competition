...
CONFIG = {
    ...
    # --- Multi-Res 配置 ---
    # 关键点：Hop Length 必须统一，保证时间维度(Time Steps)对齐
    "common_hop_length": 320, 
    "n_mels": 128,
    "resolutions": [
        {"n_fft": 512,  "win_length": 512},   # Ch1: 高时间分辨率 (捕捉瞬态：枪声、钻孔)
        {"n_fft": 1024, "win_length": 1024},  # Ch2: 平衡 (标准设置)
        {"n_fft": 2048, "win_length": 2048},  # Ch3: 高频率分辨率 (捕捉纹理：警笛、引擎)
    ],
    
    "batch_size": 112, # 显存占用会增加，稍微调小一点Batch Size
    "num_epochs": 100,
    "lr": 5e-4,     
...
# 3. [核心修改] 多分辨率预处理
# ==========================================
class MultiResLogMelPreprocessor(nn.Module):
    def __init__(self):
        super().__init__()
        self.transforms = nn.ModuleList()
        
        # 为每个分辨率创建一个 MelSpectrogram 变换
        for res in CONFIG['resolutions']:
            trans = torchaudio.transforms.MelSpectrogram(
                sample_rate=CONFIG['sample_rate'],
                n_fft=res['n_fft'],
                win_length=res['win_length'],
                hop_length=CONFIG['common_hop_length'], # 必须一致
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            ...
            # 4. 堆叠: (B, 1, F, T) * 3 -> (B, 3, F, T)
            multi_res_spec = torch.cat(batch_specs, dim=1)
...
class ConvNeXtV2_Tiny(nn.Module):
    def __init__(self, in_chans=3, num_classes=10): # 修改默认 in_chans=3
        super().__init__()
        depths = [3, 3, 9, 3]
        dims = [96, 192, 384, 768]
        self.downsample_layers = nn.ModuleList() 
        
        # Stem layer accepts 3 channels now
        stem = nn.Sequential(
...
if __name__ == "__main__":
    ...
    # 1. 实例化多分辨率预处理
    preprocessor = MultiResLogMelPreprocessor().to(CONFIG['device'])
    spec_augmenter = SpecAugmenter(CONFIG['freq_mask_param'], CONFIG['time_mask_param']).to(CONFIG['device'])
    
    # 2. 实例化模型 (in_chans=3)
    model = ConvNeXtV2_Tiny(in_chans=3, num_classes=10).to(CONFIG['device'])
    ...
    best_acc = 0.0
    print(f"\nStart Training: Multi-Res + WaveMixup + SpecAugment...")
    ...
    for epoch in range(1, CONFIG['num_epochs'] + 1):
        ...
        for wavs, labels in train_loader:
            ...
            # 2. Multi-Res Log Mel (输出 3 通道)
            specs = preprocessor(mixed_wavs) # (B, 3, F, T)
            
            # 3. SpecAugment (对 3 通道同时遮挡)
        ...
        with torch.no_grad():
            for wavs, labels in val_loader:
                # Val: 无 Mixup, 无 Augment, 但必须是 Multi-Res
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), f"best_multires_fold{FOLD}.pth")
