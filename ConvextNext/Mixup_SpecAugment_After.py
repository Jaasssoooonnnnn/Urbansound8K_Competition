...
CONFIG = {
    ...
    # [新增] SpecAugment 参数
    "freq_mask_param": 24,  # 频率遮挡最大宽度
    "time_mask_param": 80,  # 时间遮挡最大宽度
}
...
class SpecAugmenter(nn.Module):
    ...
    def forward(self, spec):
        # spec: (B, 1, F, T)
        # 必须循环处理或者合并Batch维度，因为Torchaudio的Masking通常处理单个Spec或Batch
        # Torchaudio 支持 (..., freq, time)
        with torch.no_grad():
...
if __name__ == "__main__":
    ...
    optimizer = torch.optim.AdamW(model.parameters(), lr=CONFIG['lr'], weight_decay=5e-2) # weight_decay 稍微调大一点点也有助于正则化
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=CONFIG['num_epochs'])
    criterion = nn.CrossEntropyLoss()

    best_acc = 0.0
    print(f"\nStart Training: Mixup + SpecAugment...")
    ...
    for epoch in range(1, CONFIG['num_epochs'] + 1):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), f"best_specaug_fold{FOLD}.pth")
