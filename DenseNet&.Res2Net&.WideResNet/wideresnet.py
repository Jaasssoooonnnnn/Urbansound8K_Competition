...
CONFIG = {
    ...
    "output_path": "./wideresnet50_deepstem_fusion", # 输出路径
...
# ==========================================
# 3. 主模型: WideResNet-50-2 + DeepStem
# ==========================================

class AudioWideResNetFusion(nn.Module):
    def __init__(self, num_classes=10):
        ...
        # 2. Backbone: WideResNet-50-2
        # 注意: WideResNet-50-2 的 weights=None 表示从零训练
        self.backbone = wide_resnet50_2(weights=None)
        
        # === Deep Audio Stem (关键修改) ===
        # WideResNet 默认第一层也是 7x7 stride=2，我们将其替换为 3个 3x3
        # 第一层输出通道必须是 64，以匹配 WideResNet layer1 的输入要求
        self.backbone.conv1 = nn.Sequential(
            # Layer 1: Stride 1, 32通道, 保留细节
            nn.Conv2d(1, 32, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
            # Layer 2: Stride 1, 64通道
            nn.Conv2d(32, 64, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            # Layer 3: Stride 2, 64通道, 下采样
            nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(64),  # 必须加上 BN
            nn.ReLU(inplace=True) # 必须加上 ReLU
        )
        
        # WideResNet-50-2 的最终输出通道数是 2048 (与 ResNet50 一致，只是中间层变宽了)
        out_channels = 2048
    ...
    def forward(self, x, salience=None):
        ...
        # 2. ResNet Layers
        # 注意: DeepStem 已经包含了 BN+ReLU 且做了下采样
        # 标准 ResNet forward 这里的 bn1/relu/maxpool 可以跳过或根据需要调整
        # 由于 DeepStem 替代了 conv1+bn1+relu+maxpool 的功能，我们直接进 layer1
...
if __name__ == "__main__":
    ...
    print(f"{'='*50}")
    print(f"MODEL: WideResNet-50-2 + DeepStem + SAM")
    print(f"{'='*50}")
    ...
    for fold in CONFIG['train_folds']:
        ...
        model = AudioWideResNetFusion(num_classes=10).to(CONFIG['device'])
        ...
        for epoch in range(1, CONFIG['num_epochs'] + 1):
            ...
            log_str = (f"Fold {fold} Ep {epoch:03d} | "
                       ...
                       f"Val: {val_loss:.4f} ({val_acc:.2f}%) | T: {time.time()-start:.1f}s")
    ...
    if os.path.exists(test_csv_path):
        ...
        if models_used > 0:
            ...
            save_name = os.path.join(CONFIG['output_path'], "submission_wideresnet_final.csv")
